  ...
  return (
    <SectionContainerOuter>
      <SectionContainerInner classname={"recent-albums"} title={"Recent Albums"}>
        {savedAlbums && savedAlbums?.items?.slice(0, 4).map((item: SavedAlbums) => (
          <div key={item.album.id} className="flex flex-col items-center">
            ...
            <h1 className="leading-none captalize text-sm self-start">{item.album.artists && item.album.artists[0].name}</h1>
          </div>
        ))}
