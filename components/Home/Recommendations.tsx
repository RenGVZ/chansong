  ...
  return (
    <SectionContainerOuter>
      <SectionContainerInner classname={"recommendations"} title={"Recommendations"}>
        {artistRecommendations && artistRecommendations?.tracks?.slice(0, 4).map((rec: Recommendations) => (
          <div key={rec.id} className="flex flex-col items-center">
            {
              rec?.album && <Image src={rec?.album?.images[0].url} className="rounded-[10px] max-h-[150px]" alt="rec_album_image" width={150} height={150}></Image>
            }
